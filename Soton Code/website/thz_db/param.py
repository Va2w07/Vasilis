...
def pahseUnqwarp(logW, kstart, weight, M):
    phase = logW.imag
    
    ni = 0
    lastph = phase[kstart]
    
    for i in range(kstart, -1, -1):
        diff = phase[i] - lastph
        if diff > 1.4*numpy.pi:
            ni -= 1
        if diff < -1.4*numpy.pi:
            ni += 1
        lastph = phase[i]
        phase[i] = phase[i] + 2*ni*numpy.pi

    ni = 0
    lastph = phase[kstart]
    
    for i in range(kstart, len(phase), 1):
        diff = phase[i] - lastph
        if diff > 1.4*numpy.pi:
            ni -= 1
        if diff < -1.4*numpy.pi:
    ...
    g = numpy.gradient(phase)
    ga = g[kstart]
    cg = -ga*kstart + phase[kstart]
    
#    phase = phase - cg+0.27
    
#    return logW.real + (1J)*phase
    krange = numpy.arange(0,M)
    sumw=sum(weight[0:M])
    sumwk=sum(weight[0:M]*krange)
    sumwksq=sum(weight[0:M]*krange*krange)
    sumwph=sum(weight[0:M]*phase)
    ...
    p=round(b/(2*numpy.pi))
    "original phase extrapolated without interger 2pi (no rounding)"
    #phase=phase-b
    "original phase extrapolated with interger 2pi"
    phase=phase-2*p*numpy.pi
    "straight line with gradient of the original data intercept at zero"
    #phase=a*krange
    return logW.real + (1J)*(phase)
    #return logW
